...
public class CameraPoseEstimation implements PoseEstimator{
    ...
    public CameraPoseEstimation(ICameraOdometry camera, AprilTagData aprilTagData, Transform3d cameraPosition, boolean only2TagsMeasurements){
        this.only2TagsMeasurements = only2TagsMeasurements;
        this.camera = camera;
        this.aprilTagData = aprilTagData;
        this.cameraPosition = cameraPosition;

        this.detectedPoseLogger = new CustomPose2dLogger("/Vision/PhotonVisionPoseEstimator/" + aprilTagData.getCameraName() + "/DetectedPose");
        this.numberOfDetectedTagsLogger = new CustomDoubleLogger("/Vision/PhotonVisionPoseEstimator/" + aprilTagData.getCameraName() + "/NumberOfDetectedTags");
        this.isDetectingLogger = new CustomBooleanLogger("/Vision/PhotonVisionPoseEstimator/" + aprilTagData.getCameraName() + "/IsDetecting");
        this.stateOfPoseUpdate = new CustomStringLogger("/Vision/PhotonVisionPoseEstimator/" + aprilTagData.getCameraName() + "/StateOfPoseUpdate");
    }
