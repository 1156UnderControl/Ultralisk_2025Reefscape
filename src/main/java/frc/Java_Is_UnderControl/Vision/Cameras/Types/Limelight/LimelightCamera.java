...
public class LimelightCamera implements ICameraOdometry, ICameraObject {
    ...
    public LimelightCamera(String cameraName, String objectName, double maxTx, double maxTy, Pose2d robotPose, Translation3d translation, Rotation3d rotation){
        this.cameraName = cameraName;
    ...
    private double getDistanceAprilTag(LimelightTarget_Fiducial target, int targetID) {
        ...
        if(LimelightHelpers.getTV(this.cameraName)){
            double distanceToTarget = PhotonUtils.calculateDistanceToTargetMeters(this.camToRobot.getY(), aprilTagFieldLayout.getTagPose(targetID).get().getZ(), 
    ...
    if (results.targets_Fiducials.length > 0) {
        for (var target : results.targets_Fiducials) {
            this.aprilData = new AprilTagData(
                ...
                target.ty,
                target.ta,
                this.getDistanceAprilTag(target, (int) target.fiducialID),
                this.camToRobot,
  ...
  public Optional<PoseEstimation> getRobotPose() {
    ...
    private double getDistanceObject(LimelightTarget_Retro target) {
        Pose3d targetPose = target.getTargetPose_CameraSpace();
        if(LimelightHelpers.getTV(this.cameraName)){
            double distanceToTarget = PhotonUtils.calculateDistanceToTargetMeters(this.camToRobot.getY(), this.getObjectPose().getY(), 
            Units.degreesToRadians(this.camToRobot.getX()), Units.degreesToRadians(targetPose.getY()));
            return distanceToTarget;
        } else {
            return 0;
